...
int >NoximRouter::routingFunction(const NoximRouteData & route_data)
{
    ...
    switch (NoximGlobalParams::routing_algorithm) {
        ...
        case ROUTING_LBDR:
            return routingLBDR(position, dst_coord);
...
vector<int> NoximRouter::routingLBDR(const NoximCoord& current, const NoximCoord& destination)
{
    ...
    else if (destination.y - current.y < 0)
    {
        N_prime = true;
        S_prime = false;
    }
    ...
    N_zegond = (N_prime & (!E_prime) & (!W_prime)) | (N_prime & E_prime & R_ne) | (N_prime & W_prime & R_nw);
    E_zegond = (E_prime & (!N_prime) & (!S_prime)) | (E_prime & N_prime & R_en) | (E_prime & S_prime & R_es);
    W_zegond = (W_prime & (!N_prime) & (!S_prime)) | (W_prime & N_prime & R_wn) | (W_prime & S_prime & R_ws);
    S_zegond = (S_prime & (!E_prime) & (!W_prime)) | (S_prime & E_prime & R_se) | (S_prime & W_prime & R_sw);
                            ...
                            NoximGlobalRoutingTable & grt)
{
    ...
    // Initializing LBDR bits and output signals    
}
